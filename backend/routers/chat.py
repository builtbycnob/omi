...
def send_message(
    request: Request, data: SendMessageRequest, plugin_id: Optional[str] = None, app_id: Optional[str] = None
):
...
async def create_voice_message_stream(
    request: Request, files: List[UploadFile] = File(...), language: Optional[str] = Form(None)
):
    uid = request.state.uid
    enforce_chat_quota(uid)
